...
public class CreateUserProvider {
    ...
    public void performRequest(String token, CreateUserRemoteRequest createUserRemoteRequest, Callback<UserDataRemoteResponse> createUserRemoteResponseCallback) {
        retrofit = RetrofitHelper.buildRetrofit();
        services = retrofit.create(CreateUserServices.class);

        Call<UserDataRemoteResponse> call = services.create(token, createUserRemoteRequest);
